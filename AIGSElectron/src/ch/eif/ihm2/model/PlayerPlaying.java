...
private Direction           direction;
   ...
   /**
    * Recharges the weapon with a given amount, if the weaponStatus is more than
    * 100%, the surplus is deducted.
    * 
    */
   ...
   /**
    * Returns a bullet for a fire instruction. Tests if the bullet is within the
    * playing field
    * 
    */
   public IBullet fire() {
      ...
      if (posX < 0 || posY < 0 || posY >= Constants.WORLD_HEIGHT || posX >= Constants.WORLD_WIDTH)
         return null;
    ...
    */
   public boolean hasCollided(PlayerPlaying p2) {
      ...
      for (Segment s : logicalSegments)
         if (s.onSegment(headx, heady))
            return true;
      for (Segment s : p2.logicalSegments)
         if (s.onSegment(headx, heady))
            return true;
   ...
   public void updateLogicalSegments(Segment front, Segment tail){
      ...
      // DEBUG
      if(logicalSegments.getLast().getToX() != front.getToX() || logicalSegments.getLast().getToY() != front.getToY())
         System.out.println("Head of logical segments het gicht! Is: " + logicalSegments.getLast() + " should be: " + front);
      if(tail != null){
         if(logicalSegments.getFirst().getLength() == 0)
            logicalSegments.pollFirst();
         logicalSegments.getFirst().subtract(tail);
         // DEBUG
         if(logicalSegments.getFirst().getFromX() != tail.getToX() || logicalSegments.getFirst().getFromY() != tail.getToY())
            System.out.println("Tail of logical segments het Gicht! Is: " + logicalSegments.getFirst() + " should be: " + tail);
      }
   }
   ...
   public void reset(boolean isP1) {
      ...
      if (isP1)
         ...
         first = new Segment(x, Constants.WORLD_HEIGHT, x, Constants.WORLD_HEIGHT - 1);
      segments.add(first);
