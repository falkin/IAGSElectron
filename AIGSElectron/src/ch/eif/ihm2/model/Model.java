...
import ch.eif.ai.ChromosomePlayerPlaying;
...
public class Model implements IModelOperations {
	...
	private int lastPtsP1;
	private int lastPtsP2;
	private int ptsP1;
	private int ptsP2;
	...
	private void updateHeadSegments(IPlayerPlaying p1, IPlayerPlaying p2) {
		if (p2 == null)
			gamePCS.firePropertyChange("add", null, new ISegment[] { p1.head(),
					null });
		else
			gamePCS.firePropertyChange("add", null, new ISegment[] { p1.head(),
					p2.head() });
	...
	/**
	 * Adds the listeners for the info channel, this channel is will also be
	 * used by the menu
	 * 
	 */
	...
	private void updateProgessBars(IPlayerPlaying p1, IPlayerPlaying p2) {
		infoPCS.firePropertyChange("progress1", null,
				new Integer(p1.getWeaponStatus()));
		if (p2 != null)
			infoPCS.firePropertyChange("progress2", null,
					new Integer(p2.getWeaponStatus()));
	}
	 ...
	 * fairness and avoid a situation where each player is presumed dead, but
	 * one is preferred due to a fix execution order of the collision detection
	...
	public void messageEndGame() throws CollisionDetectedException {
		if (!(p1 instanceof ChromosomePlayerPlaying && p1.equals("Ev.Train"))) {
			if (ptsP1 == ptsP2) {
	...
	/**
	 * Calculates the path of each player and draws the segments, tests if the
	 * players collide with anything on the map
	 * 
	 */
	...
	public void updateGameWin() {
		// For the training the game doesn't finish
		if (p1 instanceof ChromosomePlayerPlaying && infoP1.equals("Ev.Train")) {
			frame.setNbrWinGame(frame.getNbrWinGame() + 1);
		...
		System.out.println(ticks - lastticks);


		if (lastPtsP1 + 1 == ptsP1) {
			p1.setScore(ticks - lastticks, frame.getNbrWinGame());
			p2.setScore(-(ticks - lastticks), frame.getNbrWinGame());
		}
		if (lastPtsP2 + 1 == ptsP2) {
			p2.setScore(ticks - lastticks, frame.getNbrWinGame());
			p1.setScore(-(ticks - lastticks), frame.getNbrWinGame());
	 ...
	 * a player can move or use his weapon. Creates the commands for each
	 * player.
	 * 
	 */
	public void init(GameFrame gf) {
		...
		cellAdvancementPerTick = ((double) settings.getGameSpeed())
				/ ticksPerSecond;

		if (settings.getWeaponRechargeTime() == 0)
			weaponChargePerTick = 1.0;
		else
			weaponChargePerTick = 100.0 / (ticksPerSecond * settings
					.getWeaponRechargeTime());

		// Initialize players
		World.getInstance().init();
		if (gf.getInfoP1().equals("AI Ev.")
				|| gf.getInfoP1().equals("Ev.Train")) {
			p1 = new ChromosomePlayerPlaying("AI Ev.", settings.getAiP1()
					.getColor());
			if (gf.getInfoP1().equals("AI Ev.")) {
				((ChromosomePlayerPlaying) p1).setEvolve(false);
			}
			p1.reset(true);
		} else if (gf.getInfoP1() == "AI Fuge") {
			p1 = new FugePlayerPlaying("AI Fuge", settings.getAiP1().getColor());
			p1.reset(true);
		} else {
			p1 = new PlayerPlaying(settings.getPlayer1().getName(), settings
					.getPlayer1().getColor());
			p1.reset(true);
		}
		if (gf.isPlayer2Select() && gf.getInfoP2().equals("AI Fuge")) {
			p2 = new FugePlayerPlaying("AI Fuge", settings.getAiP2().getColor());
			p2.reset(false);
		} else if (gf.isPlayer2Select() && gf.getInfoP2().equals("AI Ev.")) {
			p2 = new ChromosomePlayerPlaying("AI Fuge", settings.getAiP2()
					.getColor());
			((ChromosomePlayerPlaying) p1).setEvolve(false);
			p2.reset(false);
		} else if (gf.isPlayer2Select()) {
			p2 = new PlayerPlaying(settings.getPlayer2().getName(), settings
					.getPlayer2().getColor());
			p2.reset(false);
		} else {
			p2 = null;
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				frame.setCommands(p1up, p1down, p1left, p1right, p2up, p2down,
						p2left, p2right, p1shoot, p2shoot);
			}
		});
